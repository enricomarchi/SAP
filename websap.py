...
def salva_sal(df):
    for index, row in df.iterrows():
        ...
        columns_with_indices = ['id_riga'] + df.columns.tolist()
...
def importa_sal_da_excel(id_sal: str, *mapping: mapping_sal, append: bool = True):
    try:
        ...
        app = xw.App(add_book=False, visible=False)
        wb = app.books.open(file_excel)
        df_sal = pd.DataFrame(columns=['id_riga', 'id_sal', 'posizione', 'id_po', 'descrizione', 'vdt', 'quantita', 'prezzo_nv', 'um_nv', 'rib', 'data_misura', 'edizione_tariffa', 'edizione_tariffa_adeguamento', 'foglio_excel', 'riga_excel', 'descrizione_vdt'])
        ...
        df_sal.insert(2, 'n_riga', df_sal.reset_index().index + inizia_da_riga)
        df_sal['id_riga'] = df_sal.apply(lambda row: f"{row['id_sal']}-{row['n_riga']:05d}", axis=1)
        df_sal.insert(0, 'id_riga', df_sal.pop('id_riga'))
        df_sal.set_index(['id_riga',], inplace=True)
